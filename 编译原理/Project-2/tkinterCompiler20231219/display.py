...
import analyze_pr
from SynAnalyzer import *
from SemAnalyzer import *
import os
    ...
    :   '  |--" |   :    ||  ,   / |   :    : `----'  |   ;/        |   :    | 
     \   \ ;     \   \  /  ---`-'   \   \  /          '---'          \   \  /  
      '---"       `----'             `----'                           `----'   
              ...
              Lexical & Syntax & Semantic Analyzer & IR generator
                                 12/19/2023
...
class FileAnalyzerApp:
    def __init__(self, root):
        ...
        # 语法分析按钮
        self.semantic_analysis_button = tk.Button(root, text="中间代码生成", command=self.semantic_analysis, width=15, state="disabled")
        self.semantic_analysis_button.grid(row=1, column=3, pady=10)
        self.semantic_analysis_button.pack_forget()
    ...
    def upload_file(self):
        ...
        if file_path:
            with open(file_path, "r") as file:
                content = file.read()
                self.file_content_analyzing = content
    ...
    def lexical_analysis(self):
        ...
        analyze_pr.lex_main(text_content)
        print("词法分析执行完毕...")
    ...
    def open_lexical_window(self):
        ...
        # 从文件中读取内容并添加到表格中
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER, analyze_pr.LexOutputFilename3), "r", encoding="utf-8") as file:
            for line in file:
                tokens = line.split()
    ...
    def syntax_analysis(self):
        ...
        if status == 0:
            ...
            close_button = tk.Button(popup, text="Close", command=popup.destroy)
            close_button.pack(pady=10)
        self.semantic_analysis_button["state"] = "normal"
    ...
    def open_syntax_window(self):
        ...
        cnt = 0
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER, analyze_pr.LexOutputFilename0), "r", encoding="utf-8") as file:
            for line in file:
                tokens = line.split()
                if cnt < 85:
        ...
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER,analyze_pr.LexOutputFilename4_display), 'r') as file:
            content = file.read()
            self.text_box.delete("1.0", tk.END)  # 清空文本框
            self.text_box.insert(tk.END, content)  # 插入文件内容
        
        tk.Label(syntax_window, text="Syntactic Grammar", font=("Helvetica", 8)).place(x=380, y=236)
    ...
    def semantic_analysis(self):
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test.txt")
        self.temp_content = self.text_display.get("1.0", tk.END)
        with open(output_path, "w") as output_file:
            output_file.write(self.temp_content.replace("main(void)", "main()"))

        analyze_pr.sem_main()
        self.open_semantic_window()
        pass

    def open_semantic_window(self):
        semantic_window = tk.Toplevel(self.root)
        semantic_window.title("语义分析和中间代码生成")
        semantic_window.geometry("1250x700")  # Set window size

        # 创建左侧文本框1（源代码）
        code_view = tk.Text(semantic_window, height=50, width=55)
        code_view.grid(row=0, column=0, padx=10, pady=20, rowspan=2) 
        on_key_release(code_view)
        code_view.bind("<Motion>", lambda event: on_key_release(code_view))
        code_view.insert(tk.END, self.temp_content)

        tk.Label(semantic_window, text="Source Code", font=("Helvetica", 8)).place(x=150, y=0)

        # 创建左侧文本框2 （FirstSet）
        new_text_box = tk.Text(semantic_window, height=20, width=55)
        new_text_box.grid(row=1, column=0, padx=10)
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER,analyze_pr.SemOutputFilename9), "r") as file66:
                new_text_box.insert(tk.END, file66.read())
        
        tk.Label(semantic_window, text="FirstSet", font=("Helvetica", 8)).place(x=150, y=680)

        # 创建中间文本框（四元式）
        Quaternion_view = tk.Text(semantic_window, height=50, width=55)
        Quaternion_view.grid(row=0, column=1, padx=10, pady=10, rowspan=2)
        on_key_release(Quaternion_view)
        # Quaternion_view.bind("<Motion>", lambda event: on_key_release(Quaternion_view))
        tk.Label(semantic_window, text="Quaternion", font=("Helvetica", 8)).place(x=570, y=0)
        try:
            with open(os.path.join(analyze_pr.OUTPUT_FOLDER,analyze_pr.SemOutputFilename7), "r") as file2:
                Quaternion_view.insert(tk.END, file2.read())
        except:
            Quaternion_view.insert(tk.END, "Quaternion Failed.")

        # 创建右侧文本框（中间代码）
        semantic_process = tk.Text(semantic_window, height=50, width=55)
        semantic_process.grid(row=0, column=2, padx=10, pady=10, rowspan=2)
        on_key_release(semantic_process)
        # semantic_process.bind("<Motion>", lambda event: on_key_release(semantic_process))
        tk.Label(semantic_window, text="Sem Stack", font=("Helvetica", 8)).place(x=970, y=0)

        try:
            with open(os.path.join(analyze_pr.OUTPUT_FOLDER,analyze_pr.SemOutputFilename8), "r") as file3:
                semantic_process.insert(tk.END, file3.read())
        except:
            semantic_process.insert(tk.END, "IR generation Failed.")
        pass

        # disable buttons except for "UPLOAD FILE"
        self.lexical_analysis_button["state"] = "disabled"
        self.syntax_analysis_button["state"] = "disabled"
        self.semantic_analysis_button["state"] = "disabled"
    ...
    def button1_action(self):
        ...
        # 从文件中读取内容并添加到表格中
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER,analyze_pr.SynOutputFilename5), "r", encoding="utf-8") as file:
            for line in file:
                tokens = line.split()
    ...
    def button2_action(self):
        ...
        # 从文件中读取内容并添加到表格中
        with open(os.path.join(analyze_pr.OUTPUT_FOLDER, analyze_pr.SynOutputFilename6), "r") as file:
            FL = False
            for line in file:
                if FL:
    ...
    def button3_action(self):
        ...
        img = Image.open(os.path.join(analyze_pr.OUTPUT_FOLDER, analyze_pr.SynTreePNG))

...
var myList = document.querySelector(".videos");

for (var i = 0; i < cv_list.length - 2; i++) {
  var newV = document.createElement("div");
  let e = i + 3;
  if (e < 10){
    e = '0' + e;
  }
  newV.innerHTML = '<i class="bx bx-play-circle"></i><i class="bx bx-pause-circle pause"></i><p class="numericalNumber">'+ e + '.</p><h3 class="title">title title title</h3><p class="time">3:46</p>'
  newV.classList.add("video");
  www = i + 2
  pt = "video-" + www;
  console.log(pt)
  newV.classList.add(pt);

  myList.appendChild(newV);
}
...
for (var i = 0; i < cv_list.length; i++) {
  fileName.push(cv_list[i].video_name)
}
...
//  let videoName = "videos/video" + i + ".mp4";
  let videoName = videoURL + i + ".mp4";
  console.log(videoURL + i + ".mp4");
